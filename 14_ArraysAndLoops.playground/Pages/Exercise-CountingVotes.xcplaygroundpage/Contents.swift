...
for votes in shouldMascotChangeVotes {
    if votes == true {yesVotes += 1 }
    else if votes == false {noVotes += 1 }
}
...
func countVotes(forIssue:String, withVotes:[Bool]) {
    noVotes = 0
    yesVotes = 0
    ...
    if yesVotes > noVotes {
        print("\(forIssue) passed \(yesVotes) to \(noVotes).")
    } else
    { print("\(forIssue) failed  \(noVotes) to \(yesVotes).") }
}

countVotes(forIssue: "Should we change the mascot?", withVotes: shouldMascotChangeVotes)
